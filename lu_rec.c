...
//	          printf("lower Number of threads = %d me hn = %d\n",omp_get_num_threads(),hn);
	#pragma omp task 
	  aux_lower_solve(a,x1,y1,a,x1+hn,y1,l,x2,y2,hn);///////////////PARALLHLIA
	#pragma omp task  			
	  aux_lower_solve(a,x1,y1+hn,a,x1+hn,y1+hn,l,x2,y2,hn);////////////////////PARALLHLIA
	#pragma omp taskwait		
}
...
void aux_upper_solve(double ** a1, int x1, int y1, double ** a2, int x2, int y2, double ** u, int x3, int y3, int N)
{//	printf("upper thread %d starting task\n",omp_get_thread_num());
...
void upper_solve(double ** a, int x1, int y1, double ** u, int x2, int y2, int N)
{
		...
		return;
     	...
     	/* Solve with recursive calls. */
	#pragma omp task 			
	  aux_upper_solve(a,x1,y1,a,x1,y1+hn,u,x2,y2,hn);///////////////PARALLHLIA
	#pragma omp task 			
	  aux_upper_solve(a,x1+hn,y1,a,x1+hn,y1+hn,u,x2,y2,hn);///////////////PARALLHLIA
	#pragma omp taskwait
}
...
void lu(double ** a, int xs, int ys, int N)
{
	...
	#pragma omp task 			//den xreiazetai na einai shared afou einai 
...
{
        int i,j;
	FILE *output_file = fopen("./out_rec","w"); 
        for (i=0;i<N;i++) {
                for (j=0;j<N;j++)
