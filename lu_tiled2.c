...
//printf("lu code: %d\n",omp_get_thread_num());            ola ta kanei ena thread afou sthn main exw grapsei single
   ...
   #pragma omp task
			...
			while(ii-k-1 >0){
				ii--;
				jj++;
//				printf("k= %d          values of ii,jj   %d , %d\n",k,ii,jj);	
   #pragma omp task
				mm_update(a,ii*B,k*B,a,k*B,jj*B,a,ii*B,jj*B,B,B,B);
			}
//			printf("**********************************\n");
		}
   #pragma omp taskwait
		/*update button right half of table*/
	...
	/*	for (i=k+1;i<range;i++) {
			...
			j=range;
			int counter;
			for (counter=mhkos_grammhs;counter > 0;counter--){
				j--;
				printf("values of i,j   %d , %d\n",i,j);	
   #pragma omp task
				{
				printf("i,j = %d , %d     executed by thread %d\n",i,j,omp_get_thread_num());
...
*/
		...
		for (i=k+1;i<range;i++) {
			mhkos_grammhs++;	
			for (j=range-mhkos_grammhs;j<range;j++){
	//			printf("k= %d          values of i,j   %d , %d\n",k,i,j);	
   #pragma omp task
				{
		//		printf("i,j = %d , %d     executed by thread %d\n",i,j,omp_get_thread_num());
				mm_update(a,i*B,k*B,a,k*B,j*B,a,i*B,j*B,B,B,B);
...
{
	int i,j;
	FILE *output_file = fopen("./out_tiled2","w");
	for (i=0;i<N;i++) {
		for (j=0;j<N;j++)
